...
function startSession()
{
    ...
    if (!isset($_SESSION['authenticated'])) {
        ...
        if (is_null($userID)) {
            ...
            if (
                !is_null($currentFirstName) && $currentFirstName != "" &&
            ...
            if (
                !is_null($currentLastName) && $currentLastName != "" &&
...
function getCurrentClasUserId()
{
    ...
    $sn = $_SERVER['sn'];
    $mail = $_SERVER['mail'];
    #$cwlLoginName = $_SERVER['cwlLoginName'];
    #$cwlLoginName = $_SERVER['persistent-id'];


    // attempt to find user by student number
    ...
    if (is_null($userID)) {
        ...
        if ($eduPersonAffiliation == "affiliate") {
            #$userID = $user->getByID($cwlLoginName);
